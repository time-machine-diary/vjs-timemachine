class ListUI {
  ...
  renderList() {
    ...
    this.renderDateBlocks(
      window.stdDateUtil.getLastDate(),
      window.stdDateUtil.getFirstDay(),
      this.list,
  ...
  renderPrevList() {
    ...
    this.renderDateBlocks(
      this.prevDateUtil.getLastDate(),
      this.prevDateUtil.getFirstDay(),
      this.prevList
    );
  ...
  renderNextList() {
    ...
    this.renderDateBlocks(
      this.nextDateUtil.getLastDate(),
      this.nextDateUtil.getFirstDay(),
      this.nextList
    );
  ...
  switchPrevCurrent() {
    ...
    this.listViewContainer.insertBefore(
      this.prevList,
      this.listViewContainer.firstElementChild
    );
  ...
  renderDateBlock(date) {
    ...
    dateBlock.selectDiary = () => {
      ...
      document.dispatchEvent(
        new CustomEvent("show-list-preview", {
          detail: {
            fileName: fileName,
            dateBlock: dateBlock
          }
    ...
    dateBlock.addEventListener("touchstart", event => {
      ...
      event.currentTarget.timer = setTimeout(() => {
        if (
          currentScrollTop === this.listViewContainer.scrollTop &&
          dateBlock.hasAttribute("assigned")
        ) {
  ...
  clearAssignedDates() {
    const assignedItems = Array.from(
      this.listViewContainer.querySelectorAll(`div.list-item[assigned]`)
    );
